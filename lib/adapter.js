...
const Adapter = {
  ...
  describe (connectionName, tableName, cb) {
    ...
    return cxn.knex(tableName).columnInfo()
      .then(result => {
        columnInfo = result
        return Promise.all([
          cxn.knex.raw(SQL.constraints, [ tableName ]),
          cxn.knex.raw(SQL.indexes, [ tableName ])
        ])
      })
      .then(results => {
        let [ constraints, indexes ] = results

        _.merge(columnInfo, 
          _.indexBy(camelize(constraints.rows), 'columnName'),
          _.indexBy(camelize(indexes.rows), 'columnName')
        )
  ...
  query (connectionName, tableName, query, _data, _cb) {
    ...
    return cxn.knex.raw(KnexUtil.toKnexRawQuery(query), KnexUtil.castValues(data))
      .then(result => {
        cb(null, result)
      })
  },
      ...
      .then(exists => {
        ...
        return connection.knex.schema.createTable(tableName, table => {
          _.each(definition, (definition, attributeName) => {
            let newColumn = KnexUtil.toKnexColumn(table, attributeName, definition)
            KnexUtil.applyColumnConstraints(newColumn, definition)
          })
          KnexUtil.applyTableConstraints(table, definition)
        })
      })
      .then(() => {
        cb()
  ...
  addAttribute (connectionName, tableName, attributeName, definition, cb) {
    ...
    return cxn.knex.schema
      .table(tableName, table => {
        let newColumn = KnexUtil.toKnexColumn(table, attributeName, definition)
        return KnexUtil.applyColumnConstraints(newColumn, definition)
      })
      .then(result => {
        cb && cb()
        return result
  ...
  create (connectionName, tableName, data, cb, _txn) {
    ...
    return KnexUtil.getTransaction(cxn, _txn)
      .then(transaction => {
        txn = transaction
        return wlsql.create(tableName, KnexUtil.sanitize(data, cxn.collections[tableName]))
      })
      .then(sql => {
        return txn.raw(KnexUtil.toKnexRawQuery(sql.query), KnexUtil.castValues(sql.values))
      })
      .then(result => {
        return KnexUtil.commitTransaction(_txn, txn, result.rows[0], cb)
      })
  ...
  createEach (connectionName, tableName, records, cb, _txn) {
    ...
    return KnexUtil.getTransaction(cxn, _txn)
      ...
      .then(result => {
        return KnexUtil.commitTransaction(_txn, txn, result, cb)
      })
      .catch(this.getErrorHandler(cb, txn))
  },
  ...
  update (connectionName, tableName, options, data, cb, _txn) {
    ...
    return KnexUtil.getTransaction(cxn, _txn)
      ...
      .then(result => {
        return KnexUtil.commitTransaction(_txn, txn, result.rows, cb)
      })
      .catch(this.getErrorHandler(cb, txn))
  },
  ...
  destroy (connectionName, tableName, options, cb) {
    ...
    return new Promise((resolve, reject) => {
      ...
      .then(sql => {
        return cxn.knex.raw(KnexUtil.toKnexRawQuery(sql.query), sql.values)
      })
      .then(result => {
        cb(null, result.rows)
      })
  ...
  find (connectionName, tableName, options, cb) {
    ...
    return new Promise((resolve, reject) => {
      ...
      .then(sql => {
        let query = KnexUtil.toKnexRawQuery(sql.query[0])
        let values = KnexUtil.castValues(sql.values[0])
