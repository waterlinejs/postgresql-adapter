...
const Adapter = {
  ...
  getPrimaryKey ({ collections }, tableName) {
    if (!collections[tableName]) {
      console.log('no definition; tableName', tableName)
    }
  ...
  find (connectionName, tableName, options, cb) {
    ...
    return new Promise((resolve, reject) => {
      ...
      .then(({ query: [query], values: [values] }) => {
        ...
        // un-fuck arguments. waterline lowercases everything which is
        // ridiculous and destroys search performance and indexability
        _.each(options.where, param => {
          const i = values.indexOf(param.toLowerCase())

          if (i !== -1) {
            values.splice(i, 1, param)
          }
        })
