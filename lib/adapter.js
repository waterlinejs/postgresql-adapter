...
const Adapter = {
  ...
  registerConnection (connection, collections, cb) {
    ...
    return this.getVersion(cxn)
      .then(version => {
        ...
        //return Util.validateVersion(version)
      ...
      .catch(err => {
        /*
        console.log('This adapter requires PostgreSQL 9.3 or higher.')
        console.log('Please either:')
        console.log('1. Upgrade your Postgres server to at least 9.3.0')
        console.log('2. Use the sails-postgresql adapter instead')
        */
        cb(err)
      })
  },
  ...
  find (connectionName, tableName, options, cb) {
    ...
    return new Promise((resolve, reject) => {
      ...
      .then(result => {
        //console.log('find results', result.rows)
        cb(null, result.rows)
      })
      .catch(this.getErrorHandler(cb))
  },
