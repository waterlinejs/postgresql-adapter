...
const Adapter = {
    ...
    /**
     * Given a list of collections, initialize wlSqlOptions.schemaName with the provided schemaName or default to public
     * @param collections
     */
    registerSchemas(collections) {
        for (let c in collections) {
            let tableName = collections[c].tableName;
            if (collections[c].schemaName) {
                Adapter.wlSqlOptions.schemaName[tableName] = collections[c].schemaName;
            } else {
                Adapter.wlSqlOptions.schemaName[tableName] = "public";
            }
        }
    },

    /**
     * Given a tableName, return the wlSqlOptions.schemaName or public if none set
     * @param tableName
     * @returns {*|string}
     */
    getSchema(tableName) {
        return Adapter.wlSqlOptions.schemaName[tableName] || "public";
    },
    /**
    ...
    registerConnection (connection, collections, cb) {
        ...
        Adapter.registerSchemas(collections);
    ...
    describe (connectionName, tableName, cb) {
        ...
        return cxn.knex(tableName)
            .withSchema(Adapter.getSchema(tableName))
            .columnInfo()
            .then(columnInfo => {
