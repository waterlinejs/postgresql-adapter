...
const Util = {
  ...
  applyPrimaryKeyConstraints (table, definition) {
    const primaryKeys = _.keys(_.pickBy(definition, attribute => {
  ...
  toKnexColumn (table, _name, attrDefinition, wlModel, schema) {
    const attr = _.isObject(attrDefinition) ? attrDefinition : {type: attrDefinition};
    let type = attr.autoIncrement ? 'serial' : attr.type;
    const name = attr.columnName || _name;
  ...
  castResultValues (values, schema) {
    return _.mapValues(values, (value, attr) => {
      const definition = schema.definition[attr];
  ...
  buildKnexJoinQuery (cxn, tableName, options) {
    ...
    const pk = Adapter.getPrimaryKey(cxn, tableName);

    const query = cxn.knex
      .select(`${tableName}.*`)
  ...
  addSelectColumns (columns, query) {
    const [oldSelectClause, fromClause] = query.split('FROM');
    const newSelectClause = [oldSelectClause.split(','), ...columns].join(',');
  ...
  buildKnexJoins (cxn, {joins}, query) {
    _.each(joins, join => {
      const parentAlias = Util.getParentAlias(join);
      const alias = Util.getSubqueryAlias(join);
      const subquery = Util.buildKnexJoinSubquery(cxn, join);
  ...
  buildKnexJoinSubquery (cxn, {criteria, child}) {
    const schema = cxn.collections[child];
  ...
  buildOrderByClause (tableName, {sort}) {
    ...
    const queryTokens = _.map(sort, (_direction, field) => {
      const direction = _direction === 1 ? '' : 'desc';
  ...
  buildWhereClause (cxn, tableName, options) {
    const parser = new CriteriaParser(tableName, cxn.schema, Adapter.wlSqlOptions);
    const {query, values} = parser.read(_.omit(options, [
  ...
  buildSelectAggregationColumns (cxn, {joins}) {
    return _.map(_.reject(joins, {select: false}), join => {
      ...
      const criteria = join.criteria || {};
      const subqueryAlias = Util.getSubqueryAlias(join);
      const asColumn = Util.getJoinAlias(join);
      const orderBy = Util.buildOrderByClause(subqueryAlias, criteria);
      const start = (criteria.skip || 0) + 1;
      const end = (criteria.limit || (Util.PG_MAX_INT - start)) + start - 1;
