...
module.exports = (function () {
  ...
  var adapter = {
    ...
    // Raw Query Interface
    query: function query(connectionName, table, _query2, data, cb) {
      ...
      spawnConnection(connectionName, function __QUERY__(client, cb) {
        ...
        // Run query
        if (data) client.query(_query2, data, cb);else client.query(_query2, cb);
      }, cb);
    },
    ...
    describe: function describe(connectionName, table, cb) {
      spawnConnection(connectionName, function __DESCRIBE__(client, cb) {
        ...
        // Get Indexes
        var indiciesQuery = 'SELECT n.nspname as "Schema", c.relname as "Name", CASE c.relkind WHEN \'r\' THEN \'table\' ' + 'WHEN \'v\' THEN \'view\' WHEN \'i\' THEN \'index\' WHEN \'S\' THEN \'sequence\' WHEN \'s\' THEN \'special\' WHEN \'f\' THEN ' + '\'foreign table\' END as "Type", pg_catalog.pg_get_userbyid(c.relowner) as "Owner", c2.relname as "Table" ' + 'FROM pg_catalog.pg_class c LEFT JOIN pg_catalog.pg_namespace n ON n.oid = c.relnamespace ' + 'LEFT JOIN pg_catalog.pg_index i ON i.indexrelid = c.oid ' + 'LEFT JOIN pg_catalog.pg_class c2 ON i.indrelid = c2.oid ' + 'WHERE c.relkind IN (\'i\',\'\') AND n.nspname <> \'pg_catalog\' AND n.nspname <> \'information_schema\' ' + 'AND n.nspname !~ \'^pg_toast\' AND pg_catalog.pg_table_is_visible(c.oid) ORDER BY 1,2;';
    ...
    define: function define(connectionName, table, definition, cb) {
      ...
      var describe = function describe(err, result) {
    ...
    // Native Join Support
    join: function join(connectionName, table, options, cb) {
      ...
      spawnConnection(connectionName, function __FIND__(client, done) {
        ...
        Cursor({
           ...
           * @param  {Function} _cb
           */
          $find: function $find(collectionName, criteria, _cb) {
           ...
           * @return {String}
           */
          $getPK: function $getPK(collectionName) {
          ...
          $populateBuffers: function populateBuffers(options, next) {
            ...
            async.auto({
              ...
              processParent: function processParent(next) {
                ...
                client.query(_query.query[0], _query.values[0], function __FIND__(err, result) {
                  ...
                  var splitChildren = function splitChildren(parent, next) {
                    var cache = {};
    ...
    // Select Query Logic
    find: function find(connectionName, table, options, cb) {
    ...
    // Count Query logic
    count: function count(connectionName, table, options, cb) {
    ...
    stream: function stream(connectionName, table, options, _stream) {
    ...
    update: function update(connectionName, table, options, data, cb) {
    ...
    destroy: function destroy(connectionName, table, options, cb) {
  ...
  function _getPK(connectionName, collectionName) {
    ...
    try {
      ...
      pk = _.find(Object.keys(collectionDefinition), function _findPK(key) {
        ...
        if (attrDef && attrDef.primaryKey) return key;else return false;
      });

      if (!pk) pk = 'id';
      return pk;
    } catch (e) {
      throw new Error('Unable to determine primary key for collection `' + collectionName + '` because ' + 'an error was encountered acquiring the collection definition:\n' + require('util').inspect(e, false, null));
    }
  }
    ...
    function after(err, client, done) {
      if (err) {
        ...
        if (tooManyConnections) {
          console.error(' -> Maybe your `poolSize` configuration is set too high?', 'e.g. If your Postgresql database only supports 20 concurrent connections, you should make sure', 'you have your `poolSize` set as something < 20 (see http://stackoverflow.com/a/27387928/486547).', 'The default `poolSize` is 10.');
          console.error('');
        }
        ...
        if (isSSLRelated) {
          console.error(' -> Are you using an SSL-enabled Postgresql host like Heroku?', 'Make sure to set `ssl` to `true` (see http://stackoverflow.com/a/22177218/486547)');
          console.error('');
        }
